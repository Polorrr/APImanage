...
export default function Register() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await client.post('/auth/register/', { username, email, password, password_confirm: password })
      ...
      if (detail?.password) {
        ...
        setError(Array.isArray(detail.username) ? detail.username[0] : detail.username)
      } else if (detail?.email) {
        setError(Array.isArray(detail.email) ? detail.email[0] : detail.email)
      } else {
        setError(detail?.message || '注册失败，请稍后重试')
      }
    } finally {
      setLoading(false)
    }
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-md mx-4 animate-float-in">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 text-sm">创建你的 AI 成本管理平台账号</p>
        </div>
